...
- In CDS & Data Definitions (.asddls, .ddls.asddls, .asbdef):
  ...
  - `DDDDLSRC` is STRICTLY for `DDLS` (Data Definitions). Query `ACMDCLSRC` for `DCLS` (Access Control), `DDLXSRC_SRC`/`DDLXSRC` for `DDLX` (Metadata Extension), and `RSBDEFSRC` for `BDEF`. Wrap in dynamic SQL and `TRY... CATCH cx_root` for cross-release safety.
- In SAP Development Objects Whitelist & UX:
  ...
  - GUI-only / metadata-only objects (`TRAN`, `NROB`, `WAPA`, `SSFO`, `SHLP`, `SRVD`) return `NOT_SUPPORTED` with empty source text; frontend defaults to `Metadata` tab with an informational `IllustratedMessage`.
  - Composite structured objects (`MSAG`, `DEVC`, `DOMA`, `DTEL`, `TTYP`) render dedicated ADT form tabs via `AdtFormParser.js`.
