...
1. Master: nhập **TR Released** → Load → `TrCmp` (badge vs `ZA026_SCORT_REPO`)
2. Click dòng → Detail mode `L_VS_T`
3. Chọn **Target REPO version** (`/Version` + `ServerType eq 'T'`) rồi `Compare`
4. Monaco: trái = REPO `TargetCode`, phải = Local Active `SourceCode`
