...
| `MX_INV_21` | **Dynamic Viewport Bounds & Zero-Clipping Rendering** | Xuất ảnh headless PNG: Bắt buộc quét `<mxGeometry>` tính $(\max_X, \max_Y)$ động. Kích thước viewport $\ge \max + 160\text{px}$, PIL auto-crop 25px uniform padding chống cắt cụt đồ hoạ. |
